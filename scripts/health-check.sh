...
check_lsp_servers() {
    ...
    if command -v pylsp >/dev/null 2>&1; then
        local pylsp_version
        pylsp_version=$(pylsp --version 2>/dev/null || echo "unknown")
        log_success "python-lsp-server available: $pylsp_version"
    else
        log_warning "python-lsp-server not found (optional)"
    fi
