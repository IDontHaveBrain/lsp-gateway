...
func (s *LeakTestMockServer) readMessage(reader *bufio.Reader) (transport.JSONRPCMessage, error) {
...
func (s *LeakTestMockServer) writeMessage(conn net.Conn, msg transport.JSONRPCMessage) error {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.clientType == "tcp" {
				...
				client, err = transport.NewStdioClient(transport.ClientConfig{
...
func TestGoroutineLeakDetection(t *testing.T) {
	...
	for cycle := 0; cycle < cycleCount; cycle++ {
		...
		var clients []transport.LSPClient
		var wg sync.WaitGroup
		...
		for i, client := range clients {
			wg.Add(1)
			go func(clientIdx int, c transport.LSPClient) {
				defer wg.Done()
