...
func TestMessageValidationError_Error(t *testing.T) {
	tests := []struct {
		name     string
		error    *mcp.MessageValidationError
		expected string
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "Context cancelled during send" {
				// testServer.cancel() // TODO: Add cancel method if needed // Cancel context just before sending
			}
	...
	tests := []struct {
		...
		},
		{
			...
			message: mcp.MCPMessage{
				...
				ID:      456,
				Result:  map[string]interface{}{"test": true},
				Error:   &mcp.MCPError{Code: -32603, Message: "Internal error"},
			},
			expectError:   true,
			errorContains: "invalid outgoing message",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "Context cancellation during recovery" {
				// server.cancel() // TODO: Add cancel method if needed // Cancel context before recovery
			}
...
func TestCallTool_NotImplementedBranch(t *testing.T) {
	...
	toolHandler.Tools["unimplemented_tool"] = mcp.Tool{
	...
	tests := []struct {
	...
	}{
		{
			...
			expectError:  true,
			expectedText: "mcp.Tool unimplemented_tool not implemented",
		},
		{
			...
			expectError:  true,
			expectedText: "mcp.Tool another_unimplemented not implemented", // This goes to the not implemented branch
		},
	}

	// Add the second unimplemented tool after creating the test cases
	toolHandler.Tools["another_unimplemented"] = mcp.Tool{
		Name:        "another_unimplemented",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// The function should not return an error for unimplemented tools, it returns a mcp.ToolResult with IsError=true
			if err != nil {
...
func TestToolHandler_EdgeCases(t *testing.T) {
	...
	t.Run("CallTool with unknown tool name", func(t *testing.T) {
		ctx := context.Background()
		result, err := toolHandler.CallTool(ctx, mcp.ToolCall{
	...
	t.Run("mcp.Tool handler initialization", func(t *testing.T) {
