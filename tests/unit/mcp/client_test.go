...
func createTestConfig() *mcp.ServerConfig {
...
func createFastTestClient(config *mcp.ServerConfig) *mcp.LSPGatewayClient {
	client := mcp.NewLSPGatewayClient(config)
	// Override retry policy for much faster tests
	client.SetRetryPolicy(&mcp.RetryPolicy{
...
func TestNewLSPGatewayClient(t *testing.T) {
	...
	if client.GetBaseURL() != config.LSPGatewayURL {
		t.Errorf("Expected baseURL %s, got %s", config.LSPGatewayURL, client.GetBaseURL())
	}

	if client.GetTimeout() != config.Timeout {
		t.Errorf("Expected timeout %v, got %v", config.Timeout, client.GetTimeout())
	}

	if client.GetMaxRetries() != config.MaxRetries {
		t.Errorf("Expected maxRetries %d, got %d", config.MaxRetries, client.GetMaxRetries())
	}
	...
	if client.GetCircuitBreakerState() != mcp.CircuitClosed {
		t.Errorf("Expected circuit breaker state to be closed, got %v", client.GetCircuitBreakerState())
	}

	if client.GetRetryPolicy().MaxRetries == 0 && config.MaxRetries > 0 {
		t.Fatal("Expected retry policy to be initialized")
	}

	if client.GetRetryPolicy().MaxRetries != config.MaxRetries {
		t.Errorf("Expected retry policy max retries %d, got %d", config.MaxRetries, client.GetRetryPolicy().MaxRetries)
	}

	if client.GetMetricsPointer() == nil {
		t.Fatal("Expected metrics to be initialized")
	}
}

func TestCircuitBreakerStates(t *testing.T) {
	cb := &mcp.CircuitBreaker{
	...
	if cb.State != mcp.CircuitHalfOpen {
...
func TestSendLSPRequestSuccess(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != mcp.JSON_RPC_ENDPOINT {
...
func TestErrorCategorization(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: mcp.ErrorCategoryNetwork,
		},
		{
			name:     "Timeout error",
			err:      fmt.Errorf("timeout exceeded"),
			expected: mcp.ErrorCategoryTimeout,
		},
		{
		...
		},
		{
			...
			expected: mcp.ErrorCategoryClient,
		},
		{
			name:     "HTTP 500 error",
			err:      fmt.Errorf("http error 500"),
			expected: mcp.ErrorCategoryServer,
		},
		{
			name:     "HTTP 429 rate limit",
			err:      fmt.Errorf("http error 429"),
			expected: mcp.ErrorCategoryRateLimit,
		},
		{
		...
		},
		{
			...
			expected: mcp.ErrorCategoryProtocol,
		},
		{
			name:     "Unknown error",
			err:      fmt.Errorf("something unexpected"),
			expected: mcp.ErrorCategoryUnknown,
		},
	}
...
func TestTimeoutHandling(t *testing.T) {
	...
	if category != mcp.ErrorCategoryTimeout {
...
func TestCircuitBreakerIntegration(t *testing.T) {
	...
	client.SetCircuitBreakerConfig(3, 5*time.Millisecond) // Reduced from 100ms
...
func TestHTTPClientConfiguration(t *testing.T) {
	...
	if client.GetHTTPClient().Timeout != config.Timeout {
		t.Errorf("Expected HTTP client timeout %v, got %v", config.Timeout, client.GetHTTPClient().Timeout)
	}
...
func TestEnhancedRetryLogic(t *testing.T) {
	...
	t.Run("ExponentialBackoffImplementation", func(t *testing.T) {
		retryPolicy := &mcp.RetryPolicy{
			...
			BackoffFactor:   2.0,
			JitterEnabled:   false,
			RetryableErrors: map[mcp.ErrorCategory]bool{mcp.ErrorCategoryServer: true},
		}
