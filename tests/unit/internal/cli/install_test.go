...
func testInstallRuntimeCommandMetadata(t *testing.T) {
	...
	if cli.GetInstallRuntimeCmd().Use == "" && cli.GetInstallRuntimeCmd().Short == "" {
		t.Skip("cli.GetInstallRuntimeCmd() appears to be uninitialized - skipping metadata test")
		return
	}

	if !strings.HasPrefix(cli.GetInstallRuntimeCmd().Use, "runtime") {
		t.Errorf("Expected Use to start with 'runtime', got '%s'", cli.GetInstallRuntimeCmd().Use)
	}

	expectedShort := "Install programming language runtimes"
	if cli.GetInstallRuntimeCmd().Short != expectedShort {
		t.Errorf("Expected Short to be '%s', got '%s'", expectedShort, cli.GetInstallRuntimeCmd().Short)
	}

	if !strings.Contains(cli.GetInstallRuntimeCmd().Long, "Supported runtimes") {
		t.Error("Expected Long description to mention supported runtimes")
	}

	if cli.GetInstallRuntimeCmd().RunE == nil {
		t.Error("Expected RunE function to be set")
	}

	if cli.GetInstallRuntimeCmd().Args == nil {
...
func testInstallRuntimeCommandValidArgs(t *testing.T) {
	...
	if len(cli.GetInstallRuntimeCmd().ValidArgs) != len(expectedValidArgs) {
		t.Errorf("Expected %d valid args, got %d", len(expectedValidArgs), len(cli.GetInstallRuntimeCmd().ValidArgs))
	}

	for i, expected := range expectedValidArgs {
		if i >= len(cli.GetInstallRuntimeCmd().ValidArgs) {
			t.Errorf("Missing valid arg: %s", expected)
			continue
		}
		if cli.GetInstallRuntimeCmd().ValidArgs[i] != expected {
			t.Errorf("Expected valid arg %d to be '%s', got '%s'", i, expected, cli.GetInstallRuntimeCmd().ValidArgs[i])
		}
	}
}
...
func testInstallRuntimeCommandArgumentValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := cli.GetInstallRuntimeCmd().Args(cli.GetInstallRuntimeCmd(), tt.args)
...
func testInstallServerCommandMetadata(t *testing.T) {
	if !strings.HasPrefix(cli.GetInstallServerCmd().Use, "server") {
		t.Errorf("Expected Use to start with 'server', got '%s'", cli.GetInstallServerCmd().Use)
	}

	expectedShort := "Install a specific language server"
	if cli.GetInstallServerCmd().Short != expectedShort {
		t.Errorf("Expected Short to be '%s', got '%s'", expectedShort, cli.GetInstallServerCmd().Short)
	}

	if !strings.Contains(cli.GetInstallServerCmd().Long, "Supported language servers") {
		t.Error("Expected Long description to mention supported language servers")
	}

	if cli.GetInstallServerCmd().RunE == nil {
		t.Error("Expected RunE function to be set")
	}

	if cli.GetInstallServerCmd().Args == nil {
		t.Error("Expected Args function to be set")
	}
}
...
func testInstallServerCommandValidArgs(t *testing.T) {
	...
	if len(cli.GetInstallServerCmd().ValidArgs) != len(expectedValidArgs) {
		t.Errorf("Expected %d valid args, got %d", len(expectedValidArgs), len(cli.GetInstallServerCmd().ValidArgs))
	}

	for i, expected := range expectedValidArgs {
		if i >= len(cli.GetInstallServerCmd().ValidArgs) {
			t.Errorf("Missing valid arg: %s", expected)
			continue
		}
		if cli.GetInstallServerCmd().ValidArgs[i] != expected {
			t.Errorf("Expected valid arg %d to be '%s', got '%s'", i, expected, cli.GetInstallServerCmd().ValidArgs[i])
		}
	}
}
...
func testInstallServerCommandArgumentValidation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := cli.GetInstallServerCmd().Args(cli.GetInstallServerCmd(), tt.args)
