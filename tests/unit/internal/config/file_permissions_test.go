...
func TestConfigFileReadPermissionDenied(t *testing.T) {
	...
	t.Run("config_file_no_read_permission", func(t *testing.T) {
		...
		configContent := configpkg.TEST_BASIC_GO_CONFIG
	...
	t.Run("config_file_no_permissions", func(t *testing.T) {
		...
		configContent := configpkg.TEST_EMPTY_SERVERS_CONFIG
	...
	t.Run("config_file_execute_only_permission", func(t *testing.T) {
		if runtime.GOOS == configpkg.PLATFORM_WINDOWS {
...
func testWindowsFileAttributes(t *testing.T) {
	...
	createConfigFile(t, configFile, configpkg.TEST_EMPTY_SERVERS_CONFIG)
...
func validateSuccessfulRead(t *testing.T, config *configpkg.GatewayConfig, err error, description string, mode os.FileMode) {
	if err != nil {
