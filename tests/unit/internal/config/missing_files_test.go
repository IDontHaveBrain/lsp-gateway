...
func TestLoadConfig_MissingConfigFile(t *testing.T) {
	...
	_, err := configpkg.LoadConfig(nonExistentPath)
...
func TestLoadConfig_MissingConfigDirectory(t *testing.T) {
	...
	_, err := configpkg.LoadConfig(missingDirPath)
...
func TestLoadConfig_PermissionDenied(t *testing.T) {
	...
	_, loadErr := configpkg.LoadConfig(configFile)
	if loadErr == nil {
...
func TestLoadConfig_EmptyConfigFile(t *testing.T) {
	...
	config, loadErr := configpkg.LoadConfig(configFile)
	if loadErr == nil {
...
func TestValidateConfig_MissingRequiredSections(t *testing.T) {
	...
	tests := []struct {
		name    string
		config  *configpkg.GatewayConfig
		wantErr bool
		errMsg  string
	}{
		{
		...
		},
		{
			...
			config: &configpkg.GatewayConfig{
				Port:    8080,
				Servers: []configpkg.ServerConfig{},
			},
			wantErr: true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := configpkg.ValidateConfig(tt.config)
...
`
	...
	validateErr := configpkg.ValidateConfig(config)
	if validateErr == nil {
...
func TestLoadConfig_FileRecoveryScenarios(t *testing.T) {
	...
	_, err1 := configpkg.LoadConfig(configFile)
	if err1 == nil {
	...
	config, err2 := configpkg.LoadConfig(configFile)
	if err2 != nil {
	...
	_, err3 := configpkg.LoadConfig(configFile)
	if err3 == nil {
...
func TestLoadConfig_DirectoryPermissionScenarios(t *testing.T) {
	...
	_, loadErr2 := configpkg.LoadConfig(configFile)
...
`
	...
	if config.Servers[0].Transport != "stdio" {
		t.Errorf("Expected default transport '%s', got '%s'", "stdio", config.Servers[0].Transport)
	}
}
...
func BenchmarkLoadConfig_MissingFile(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = configpkg.LoadConfig(nonExistentPath)
	}
}
...
func BenchmarkLoadConfig_ExistingFile(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = configpkg.LoadConfig(configFile)
	}
}
...
func BenchmarkValidateConfig_CompleteConfig(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = configpkg.ValidateConfig(config)
	}
}
