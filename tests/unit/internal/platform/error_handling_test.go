...
func TestPlatformError(t *testing.T) {
	...
	platformErr := platform.NewPlatformError(platform.ErrCodePackageManagerFailed, "test operation failed", "test problem", "test suggestion")
	...
	otherPlatformErr := platform.NewPlatformError(platform.ErrCodePackageManagerFailed, "other message", "other problem", "other suggestion")
	...
	differentPlatformErr := platform.NewUnsupportedPlatformError("custom-os")
...
func TestPlatformErrorNilCause(t *testing.T) {
	platformErr := platform.NewPlatformError(platform.ErrCodeInsufficientPermissions, "test without cause", "test problem", "test suggestion")
	...
	otherErr := platform.NewUnsupportedPlatformError("custom-os")
...
func TestExecutionError(t *testing.T) {
	...
	execErr := platform.NewExecutionError("ls", 1, "permission denied", baseErr)
...
func TestExecutionErrorZeroExitCode(t *testing.T) {
	execErr := platform.NewExecutionError("echo", 0, "test output", nil)
...
func TestPackageManagerError(t *testing.T) {
	...
	pkgErr := platform.NewPackageManagerError("apt", "install", "nonexistent-package", "package not found", baseErr)
...
func TestUnsupportedPlatformError(t *testing.T) {
	unsupportedErr := platform.NewUnsupportedPlatformError("custom-os")
...
func TestInsufficientPermissionsError(t *testing.T) {
	permErr := platform.NewInsufficientPermissionsError("install package", "/usr/local/bin")
...
func TestInsufficientPermissionsErrorEmptyPath(t *testing.T) {
	permErr := platform.NewInsufficientPermissionsError("restricted operation", "")
...
func TestErrorChaining(t *testing.T) {
	...
	execErr := platform.NewExecutionError("failed-command", 1, "stderr output", rootErr)
	platformErr := platform.NewPlatformError(platform.ErrCodePackageManagerFailed, "command execution failed", "problem", "suggestion")
...
func TestErrorEdgeCases(t *testing.T) {
	...
	emptyPlatformErr := platform.NewPlatformError(platform.ErrCodeUnsupportedPlatform, "", "", "")
	...
	longErr := platform.NewPlatformError(platform.ErrCodePackageManagerFailed, string(longMessage), "problem", "suggestion")
	if len(longErr.Error()) == 0 {
		t.Error("Long message error should not be empty")
	}

	// Test execution error with empty command
	emptyExecErr := platform.NewExecutionError("", 0, "", nil)
	...
	emptyPkgErr := platform.NewPackageManagerError("", "", "", "", nil)
