...
func TestGetHomeDirectoryWindowsLogic(t *testing.T) {
	...
	t.Run("USERPROFILE priority over HOMEDRIVE+HOMEPATH", func(t *testing.T) {
		...
		result, err := platform.GetHomeDirectory()
		if err != nil {
...
func TestSupportsShellWindowsSpecific(t *testing.T) {
	...
	for _, test := range windowsShells {
		t.Run(test.shell, func(t *testing.T) {
			result := platform.SupportsShell(test.shell)
			if result != test.expected {
				t.Errorf("platform.SupportsShell(%q) on Windows: expected %v, got %v",
...
func TestSupportsShellUnixSpecific(t *testing.T) {
	...
	for _, test := range unixShells {
		t.Run(test.shell, func(t *testing.T) {
			result := platform.SupportsShell(test.shell)
			if result != test.expected {
				t.Errorf("platform.SupportsShell(%q) on Unix: expected %v, got %v",
					test.shell, test.expected, result)
			}
		})
	}
...
func TestSupportsShellCaseInsensitive(t *testing.T) {
	...
	for _, shell := range caseMixtures {
		t.Run(shell, func(t *testing.T) {
			result := platform.SupportsShell(shell)

			// Verify case insensitivity works
			lowercase := strings.ToLower(shell)
			expectedResult := platform.SupportsShell(lowercase)
...
func TestSupportsShellAdditionalCases(t *testing.T) {
	edgeCases := []struct {
		...
		{"./sh", platform.IsUnix(), "relative path sh"},
		{"../bash", platform.IsUnix(), "parent directory bash"},
		{"C:\\cmd.exe", platform.IsWindows(), "Windows absolute path"},
		{"\\windows\\system32\\cmd.exe", platform.IsWindows(), "Windows path without drive"},
	}
	...
	for _, test := range edgeCases {
		t.Run(test.reason, func(t *testing.T) {
			result := platform.SupportsShell(test.shell)
			if result != test.expected {
				t.Errorf("platform.SupportsShell(%q): expected %v, got %v (%s)",
