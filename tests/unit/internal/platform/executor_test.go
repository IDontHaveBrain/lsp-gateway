...
func TestNewCommandExecutor(t *testing.T) {
	...
	// Skip tests for private executor types as they cannot be accessed from external packages
...
func TestExecuteBasicCommand(t *testing.T) {
	...
	if runtime.GOOS == string(platform.PlatformWindows) {
		cmd = "cmd"
...
func TestIsCommandAvailable(t *testing.T) {
	...
	if !platform.IsCommandAvailable(knownCommand) {
		t.Errorf("Expected '%s' to be available", knownCommand)
	}

	if platform.IsCommandAvailable("nonexistent_command_xyz123") {
...
func TestExecuteShellCommand(t *testing.T) {
	...
	result, err := platform.ExecuteShellCommand(executor, command, 5*time.Second)
	if err != nil {
