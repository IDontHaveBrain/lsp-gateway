...
func TestGoDetectorConfiguration(t *testing.T) {
	...
	// Test timeout setting - we can only test that it doesn't panic
	// since timeout field is not accessible via interface
	...
	t.Logf("  Timeout set successfully (value not accessible via interface)")
