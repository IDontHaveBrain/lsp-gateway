...
func TestErrorUnwrapMethods(t *testing.T) {
	t.Run("InstallerError Unwrap", func(t *testing.T) {
		...
		installerErr := installer.NewInstallerError(installer.InstallerErrorTypeInstallation, "test", "failed", baseErr)
	...
	t.Run("InstallerError Is", func(t *testing.T) {
		...
		targetErr := installer.NewInstallerError(installer.InstallerErrorTypeInstallation, "other", "failed", nil)
		if !installerErr.Is(targetErr) {
			t.Error("Expected Is() to return true for same error type")
		}

		otherErr := installer.NewInstallerError(installer.InstallerErrorTypeVerification, "other", "failed", nil)
	...
	t.Run("InstallationError methods", func(t *testing.T) {
		...
		installErr := installer.NewInstallationError("nodejs", "download", 1, "failed", baseErr)
	...
	t.Run("VerificationError methods", func(t *testing.T) {
		...
		verifyErr := installer.NewVerificationError("java", "version", "17.0.0", "1.8.0", baseErr)
...
func TestValidateVersionMethod(t *testing.T) {
	installer := installer.NewRuntimeInstaller()
...
// TestDetectCurrentPlatformMethod tests platform detection using runtime.GOOS
func TestDetectCurrentPlatformMethod(t *testing.T) {
	...
	if !isValid {
		t.Logf("Unexpected platform (but may be valid): '%s'", platform)
	}
