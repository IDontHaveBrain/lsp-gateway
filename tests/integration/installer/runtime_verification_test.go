...
func TestVerifyJavaEnvironment_ValidJavaHome(t *testing.T) {
	...
	if result.EnvironmentVars[installer.ENV_VAR_JAVA_HOME] != "/tmp" {
		t.Errorf("Expected JAVA_HOME to be set to '/tmp', got '%s'", result.EnvironmentVars[installer.ENV_VAR_JAVA_HOME])
	}
}
