...
func TestClientCreation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			client, err := transport.NewLSPClient(tt.config)
...
func TestTransportTypeVerification(t *testing.T) {
	stdioConfig := transport.ClientConfig{
		Command:   "cat",
		Transport: "stdio",
	}

	stdioClient, err := transport.NewLSPClient(stdioConfig)
	if err != nil {
		t.Fatalf("Failed to create STDIO client: %v", err)
	}

	if _, ok := stdioClient.(*transport.StdioClient); !ok {
		t.Error("STDIO transport should return StdioClient")
	}

	tcpConfig := transport.ClientConfig{
		Command:   fmt.Sprintf("localhost:%d", testutil.AllocateTestPort(t)),
		Transport: transport.TransportTCP,
	}

	tcpClient, err := transport.NewLSPClient(tcpConfig)
	if err != nil {
		t.Fatalf("Failed to create TCP client: %v", err)
	}

	if _, ok := tcpClient.(*transport.TCPClient); !ok {
		t.Error("TCP transport should return TCPClient")
	}
}

func TestClientInterfaceCompliance(t *testing.T) {
	var _ transport.LSPClient = &transport.StdioClient{}
	var _ transport.LSPClient = &transport.TCPClient{}

	configs := []transport.ClientConfig{
		{
			Command:   "cat",
			Transport: "stdio",
		},
...
func TestProtocolCompliance(t *testing.T) {
	...
	t.Run("TCP_JSON_RPC_Format", func(t *testing.T) {
		...
		go func() {
			defer close(done)
			testJSONRPCFormatWithTimeout(t, transport.TransportTCP, ctx)
		}()
...
func createClientOrFail(t *testing.T, config transport.ClientConfig) transport.LSPClient {
	client, err := transport.NewLSPClient(config)
	if err != nil {
		t.Fatalf("transport.NewLSPClient failed: %v", err)
	}
	return client
}
...
func stopClientSafely(t *testing.T, client transport.LSPClient) {
	if err := client.Stop(); err != nil {
...
func testSingleStartStopCycle(t *testing.T, config transport.ClientConfig, cycleNum int) {
	client, err := transport.NewLSPClient(config)
	if err != nil {
		t.Fatalf("transport.NewLSPClient failed on cycle %d: %v", cycleNum, err)
	}
...
func verifyClientActive(t *testing.T, client transport.LSPClient, cycleNum int, shouldBeActive bool) {
...
func testClientLifecycle(t *testing.T, config transport.ClientConfig) {
	client, err := transport.NewLSPClient(config)
	if err != nil {
		t.Fatalf("transport.NewLSPClient failed: %v", err)
	}
...
func TestCrossTransportComparison(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			tcpResult := testClientBehavior(t, transport.TransportTCP, tc.method, tc.params)
...
func createMockStdioEchoClient(t *testing.T) (transport.LSPClient, error) {
	config := transport.ClientConfig{
		Command:   "cat",
		Args:      []string{},
		Transport: "stdio",
...
func createMockTCPEchoServer(t *testing.T) (transport.LSPClient, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	return createMockTCPEchoServerWithTimeout(t, ctx)
}

func createMockTCPEchoServerWithTimeout(t *testing.T, ctx context.Context) (transport.LSPClient, error) {
...
func createEchoResponse(msg transport.JSONRPCMessage) transport.JSONRPCMessage {
	return transport.JSONRPCMessage{
		JSONRPC: "2.0",
		ID:      msg.ID,
...
func sendLSPResponse(writer *bufio.Writer, response transport.JSONRPCMessage) error {
...
func createTCPClient(addr string) (transport.LSPClient, error) {
	config := transport.ClientConfig{
		Command:   addr,
		Transport: transport.TransportTCP,
	}
