...
func TestNewLinuxStrategy(t *testing.T) {
	...
	strategy, err := installer.NewLinuxStrategy()
	if err != nil {
...
func TestPackageManagerAvailability(t *testing.T) {
	...
	// Test with available package managers from platform
	...
	if len(available) == 0 {
		t.Log("No package managers are available on this Linux system (may be expected in test environment)")
	}
}
