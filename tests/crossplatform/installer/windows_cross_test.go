...
func TestWindowsStrategyCrossCompilation(t *testing.T) {
	t.Run("WindowsStrategyCreation", func(t *testing.T) {
		strategy := installer.NewWindowsStrategy()
		if strategy == nil {
	...
	t.Run("WindowsStrategyInterface", func(t *testing.T) {
		var strategy installer.PlatformStrategy = installer.NewWindowsStrategy()
...
func (m *mockWindowsPackageManager) Install(component string) error {
	...
	if m.shouldFail {
		return &installer.InstallerError{
			Type:      installer.InstallerErrorTypeInstallation,
			Component: component,
