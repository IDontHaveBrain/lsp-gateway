...
func validateCommandResult(t *testing.T, result *platform.Result, expectedOutput string) {
...
func testCommandAvailability(t *testing.T, executor platform.CommandExecutor, commands []string) {
...
func validateTimeout(t *testing.T, elapsed time.Duration, err error, result *platform.Result) {
...
// isShellSupported checks if a shell is supported on the current platform
func isShellSupported(shell string) bool {
	...
	case "windows":
		...
		for _, supported := range windowsShells {
			if strings.Contains(shell, supported) {
				return true
			}
		}
	case "linux", "darwin":
	...
	t.Run("PlatformShellSupport", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.shell, func(t *testing.T) {
				supported := isShellSupported(tc.shell)
		...
		for shell, paths := range pathVariations {
			t.Run(shell, func(t *testing.T) {
				for _, path := range paths {
					supported := isShellSupported(path)
...
func validateInvalidArgumentsResult(t *testing.T, err error, result *platform.Result) {
	if err == nil && (result == nil || result.ExitCode == 0) {
...
func executeConcurrentCommands(executor platform.CommandExecutor, cmd string, args []string, numCommands int) []concurrentResult {
