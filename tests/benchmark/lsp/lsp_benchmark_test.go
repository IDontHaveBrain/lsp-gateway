...
func benchmarkLSPMethod(b *testing.B, method string) {
	fw := setupLSPBenchmarkFramework(b)

	result := fw.BenchmarkMethod(context.Background(), b, method)
...
func benchmarkLSPMethodMemory(b *testing.B, method string) {
	...
	if result.MemoryMetrics.AllocPerRequest > fw.Config.MemoryThresholds.MaxAllocPerRequest {
		b.Errorf("Memory allocation per request %d bytes exceeds threshold %d bytes",
			result.MemoryMetrics.AllocPerRequest, fw.Config.MemoryThresholds.MaxAllocPerRequest)
	}

	if result.MemoryMetrics.MemoryGrowthPercent > fw.Config.MemoryThresholds.MaxMemoryGrowth {
		b.Errorf("Memory growth %.1f%% exceeds threshold %.1f%%",
			result.MemoryMetrics.MemoryGrowthPercent, fw.Config.MemoryThresholds.MaxMemoryGrowth)
	}
}
...
func benchmarkLSPMethodThroughput(b *testing.B, method string) {
	...
	if throughput < fw.Config.ThroughputThresholds.MinRequestsPerSecond {
		b.Errorf("Throughput %.2f req/sec below threshold %.2f req/sec",
			throughput, fw.Config.ThroughputThresholds.MinRequestsPerSecond)
	}
...
func benchmarkLSPMethodLatency(b *testing.B, method string) {
	...
	if len(latencies) > 0 {
		metrics := framework.CalculateLatencyMetrics(latencies)
...
func setupLSPBenchmarkFramework(b *testing.B) *framework.LSPBenchmarkFramework {
	options := &framework.FrameworkOptions{
		ConfigPath:   "tests/data/configs/performance-test-config.yaml",
	...
	benchmarkConfig := framework.DefaultBenchmarkConfig()
	// Override for testing
	benchmarkConfig.WarmupIterations = 50
	benchmarkConfig.BenchmarkDuration = 10 * time.Second

	framework, err := framework.NewLSPBenchmarkFramework(options, benchmarkConfig)
	if err != nil {
