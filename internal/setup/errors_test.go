...
func TestInstallationError(t *testing.T) {
	err := NewRuntimeNotFoundError("go").
		WithVersion("1.21.0").
...
func TestValidationError(t *testing.T) {
	err := NewConfigValidationFailedError("port", "invalid", "must be between 1 and 65535").
...
func TestCommandError(t *testing.T) {
	err := NewCommandError(ErrCodeCommandFailed, "git", []string{"clone", "repo"}, 128).
...
func TestProgressInfo(t *testing.T) {
	progress := NewProgressInfo("test-operation", 100)
...
func TestRetryableErrors(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			err:      NewNetworkError("download", "http://example.com", fmt.Errorf("timeout")),
			expected: true,
		},
		{
			name:     "Command timeout should be retryable",
			err:      NewCommandError(ErrCodeCommandTimeout, "curl", []string{}, 124),
			expected: true,
		},
		{
			name:     "Validation error should not be retryable",
			err:      NewConfigValidationFailedError("port", "invalid", "must be numeric"),
			expected: false,
		},
		{
...
func BenchmarkProgressUpdate(b *testing.B) {
	progress := NewProgressInfo("benchmark", 1000000)
