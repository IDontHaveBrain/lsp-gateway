...
func TestWindowsStrategy_InstallGo_ComprehensiveScenarios(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expectedError:       true,
			expectedErrorType:   InstallerErrorTypeUnsupported,
		},
		{
			...
			expectedError:     true,
			expectedErrorType: InstallerErrorTypeInstallation,
		},
		{
...
func TestLinuxStrategy_InstallComponent_AllManagersFail(t *testing.T) {
	...
	if !ok {
		...
		if installerErr.Type != InstallerErrorTypeInstallation {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedError {
				...
				} else if installerErr, ok := err.(*InstallerError); ok {
					if installerErr.Type != tc.expectedErrorType {
						t.Errorf("Expected error type %s, got %s", tc.expectedErrorType, installerErr.Type)
					}
				}
...
func createTestWindowsStrategyWithConfig(wingetAvailable, chocoAvailable bool) *WindowsStrategy {
...
func createTestLinuxStrategy(linuxInfo *platform.LinuxInfo) *LinuxStrategy {
...
func createTestMacOSStrategy() *MacOSStrategy {
...
func TestPlatformStrategy_ErrorHandling_Comprehensive(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectedType:  InstallerErrorTypeInstallation,
		},
		{
