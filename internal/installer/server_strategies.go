...
const (
	JDTLSVersion        = "1.43.0"
	JDTLSTimestamp      = "202412191447"
	JDTLSChecksum       = "e3a33ff888aaf1d998ec0b6e0f1ecb8eeac53eef88dde22204c3cd3379e5fb98"
...
// GetJava21InstallPath returns the installation path for Java 21 runtime based on the platform
func GetJava21InstallPath() string {
	switch runtime.GOOS {
	case string(platform.PlatformWindows):
		if appData := os.Getenv("APPDATA"); appData != "" {
			return filepath.Join(appData, "lsp-gateway", "java21")
		}
		return filepath.Join(os.Getenv("USERPROFILE"), ".lsp-gateway", "java21")
	case "darwin":
		home := os.Getenv("HOME")
		return filepath.Join(home, "Library", "Application Support", "lsp-gateway", "java21")
	default: // linux and others
		if xdgDataHome := os.Getenv("XDG_DATA_HOME"); xdgDataHome != "" {
			return filepath.Join(xdgDataHome, "lsp-gateway", "java21")
		}
		home := os.Getenv("HOME")
		return filepath.Join(home, ".local", "share", "lsp-gateway", "java21")
	}
}

// GetJava21ExecutablePath returns the path to the Java 21 executable
func GetJava21ExecutablePath() string {
	installPath := GetJava21InstallPath()
	
	if runtime.GOOS == "windows" {
		return filepath.Join(installPath, "bin", "java.exe")
	}
	return filepath.Join(installPath, "bin", "java")
}

// checkJava21Available checks if the dedicated Java 21 runtime is available
func checkJava21Available() bool {
	javaPath := GetJava21ExecutablePath()
	_, err := os.Stat(javaPath)
	return err == nil
}

// ensureJava21Runtime ensures that Java 21 runtime is installed and available
func (u *UniversalServerStrategy) ensureJava21Runtime() error {
	// Check if Java 21 is already available
	if checkJava21Available() {
		return nil
	}

	// Create Java 21 runtime manager and install
	javaManager := NewJava21RuntimeManager()
	
	// Download and install Java 21 runtime
	if err := javaManager.DownloadAndInstall(false); err != nil {
		return fmt.Errorf("failed to install Java 21 runtime: %w", err)
	}

	// Verify installation
	if err := javaManager.VerifyInstallation(); err != nil {
		return fmt.Errorf("Java 21 runtime installation verification failed: %w", err)
	}
...
func (u *UniversalServerStrategy) installJDTLS(options types.ServerInstallOptions, start time.Time) (*types.InstallResult, error) {
	...
	// Ensure Java 21 runtime is available before installing JDTLS
	if err := u.ensureJava21Runtime(); err != nil {
		return &types.InstallResult{
			Success:  false,
			Runtime:  ServerJDTLS,
			Duration: time.Since(start),
			Errors:   []string{fmt.Sprintf("Failed to ensure Java 21 runtime: %v", err)},
			Details:  map[string]interface{}{"server": ServerJDTLS, "java21_required": true},
		}, err
	}
...
func (u *UniversalServerStrategy) createWindowsJDTLSScript(binDir, jarPath string, platform string) error {
	...
	java21Path := strings.ReplaceAll(GetJava21ExecutablePath(), "/", "\\")
	
	// Validate Java 21 path is not empty
	if java21Path == "" {
		return fmt.Errorf("failed to get Java 21 executable path for Windows script generation")
	}
...
`, java21Path, java21Path, jarPath, jarPath, configDir, jarPath, configDir)
...
func (u *UniversalServerStrategy) createUnixJDTLSScript(binDir, jarPath string, platform string) error {
	...
	// Get dedicated Java 21 path
	java21Path := GetJava21ExecutablePath()
	
	// Validate Java 21 path is not empty
	if java21Path == "" {
		return fmt.Errorf("failed to get Java 21 executable path for Unix script generation")
	}
