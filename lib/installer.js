...
class Installer {
  ...
  getExpectedChecksum() {
    ...
    const platformKey = this.platform.getCurrentPlatform();
  ...
  async verifyBinaryChecksum(binaryPath) {
    ...
    if (!expectedChecksum) {
      console.log(`⚠️  No checksum available for version ${this.version} on ${this.platform.getCurrentPlatform()}`);
  ...
  async install() {
    try {
      ...
      if (!this.platform.isSupported()) {
        throw new Error(`Platform ${this.platform.getCurrentPlatform()} is not supported`);
      }
  ...
  async testBinary(binaryPath) {
    return new Promise((resolve) => {
      ...
      child.on('close', (code) => {
        ...
        const success = code === 0 && (output.includes('lsp-gateway') || output.includes('version') || output.includes('dev') || output.trim().length > 0);
        if (success) {
  ...
  async handleInstallationError(error) {
    ...
    console.error(`  Current: ${this.platform.getCurrentPlatform()}`);
