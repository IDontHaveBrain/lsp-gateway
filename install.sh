...
if [ "$SHOW_HELP" = true ]; then
    ...
    echo "  - make"
    echo ""
    echo "After installation, you can use 'lsp-gateway' command globally."
    exit 0
fi
...
if [ "$DRY_RUN" = true ]; then
    print_dry_run "command -v lsp-gateway (check if lsp-gateway command is available)"
    print_dry_run "lsp-gateway version (test lsp-gateway command)"
else
    # Check if lsp-gateway command is available
    if command_exists lsp-gateway; then
        print_success "lsp-gateway command is available"
        
        # Test lsp-gateway version command
        if lsp-gateway version >/dev/null 2>&1; then
            INSTALLED_VERSION=$(lsp-gateway version 2>/dev/null | grep "Version:" | awk '{print $2}' || echo "unknown")
            print_success "lsp-gateway version test passed - Version: $INSTALLED_VERSION"
        else
            print_warning "lsp-gateway command exists but version test failed"
        fi
    else
        print_error "lsp-gateway command not found in PATH"
...
print_info "You can now use the 'lsp-gateway' command:"
print_info "  lsp-gateway version      - Show version information"
print_info "  lsp-gateway server       - Start LSP Gateway server"
print_info "  lsp-gateway diagnose     - Run diagnostics"
print_info "  lsp-gateway setup all    - Setup language servers"
echo
print_info "For more commands, run: lsp-gateway --help"
