...
func TestMCPToolCallTimeouts(t *testing.T) {
	...
	for _, tool := range tools {
		t.Run(tool.toolName, func(t *testing.T) {
			...
			start := time.Now()
			result, err := toolHandler.CallTool(ctx, ToolCall{Name: tool.toolName, Arguments: tool.args})
			duration := time.Since(start)
			...
			if duration < 500*time.Millisecond || duration > 3*time.Second {
				t.Errorf("Tool %s timeout at %v, expected around 1s", tool.toolName, duration)
			}
...
func TestMCPConcurrentTimeouts(t *testing.T) {
	...
	for i, toolName := range tools {
		...
		go func(name string, id int) {
			...
			start := time.Now()
			_, err := toolHandler.CallTool(ctx, ToolCall{Name: name, Arguments: args})
			duration := time.Since(start)
	...
	for result := range results {
		...
		if result.duration < 500*time.Millisecond || result.duration > 2*time.Second {
			t.Errorf("Tool %s timeout at %v, expected around 1s", result.toolName, result.duration)
		}
	}
