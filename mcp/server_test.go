...
func (m *MockToolHandler) ListTools() []Tool {
	m.mu.RLock()
	defer m.mu.RUnlock()
	return append([]Tool{}, m.tools...)
}

func (m *MockToolHandler) CallTool(ctx context.Context, call ToolCall) (*ToolResult, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	...
	return &ToolResult{
		Content: []ContentBlock{{
			Type: "text",
			Text: fmt.Sprintf("Mock result for tool: %s", call.Name),
		}},
		IsError: false,
	}, nil
}

func (m *MockToolHandler) AddTool(tool Tool) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.tools = append(m.tools, tool)
}

func (m *MockToolHandler) SetResult(toolName string, result *ToolResult) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.results[toolName] = result
}

func (m *MockToolHandler) SetError(toolName string, err error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.errors[toolName] = err
}

func (m *MockToolHandler) SetDelay(toolName string, delay time.Duration) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func TestErrorScenarios(t *testing.T) {
	...
	t.Run("InvalidToolCallParams", func(t *testing.T) {
		...
		if !strings.Contains(output, "Tool name is required") {
			t.Error("Expected 'Tool name is required' error")
		}
	})
}
