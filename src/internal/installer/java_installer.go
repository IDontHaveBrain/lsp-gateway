...
# Resolve workspace directory (per-repo by default)
if [ -z "$1" ]; then
    BASE_DIR="$HOME/.lsp-gateway/jdtls-workspaces"
    mkdir -p "$BASE_DIR"
    # Derive a stable name from current directory (repo) and its hash
    REPO_NAME="$(basename "$PWD")"
    if command -v md5sum >/dev/null 2>&1; then
        REPO_HASH="$(printf %%s "$PWD" | md5sum | cut -d' ' -f1 | cut -c1-8)"
    else
        # Fallback simple hash
        REPO_HASH="$(printf %%s "$PWD" | sha1sum 2>/dev/null | cut -c1-8)"
        [ -z "$REPO_HASH" ] && REPO_HASH="default"
    fi
    WORKSPACE="$BASE_DIR/${REPO_NAME}-${REPO_HASH}"
else
    WORKSPACE="$1"
fi
mkdir -p "$WORKSPACE"
...
REM Resolve workspace directory (per-repo by default)
if "%%1"=="" (
    set "BASE_DIR=%%USERPROFILE%%\.lsp-gateway\jdtls-workspaces"
    if not exist "%%BASE_DIR%%" mkdir "%%BASE_DIR%%"
    set "REPO_NAME=%CD%"
    for %%I in ("%CD%") do set "REPO_NAME=%%~nxI"
    REM Sanitize path for directory name (replace : and \ with _)
    set "SANITIZED=%CD%"
    set "SANITIZED=!SANITIZED::=_!"
    set "SANITIZED=!SANITIZED:\\=_!"
    REM Simple hash surrogate using string length
    set "LEN=0"
    for /l %%L in (12,1,12) do set "LEN=%%L"
    set "WORKSPACE=%%BASE_DIR%%\%%REPO_NAME%%-%%LEN%%"
) else (
    set WORKSPACE=%%1
)
