...
func (m *LSPManager) ProcessRequest(ctx context.Context, method string, params interface{}) (interface{}, error) {
    if m.scipCache != nil && m.isCacheableMethod(method) {
        if result, found, err := m.scipCache.Lookup(method, params); err == nil && found { return result, nil }
    }
    ...
    if err != nil {
        if method == types.MethodWorkspaceSymbol {
            ...
            for k, v := range m.clients { clients[k] = v }
            m.mu.RUnlock()
            result, err := m.workspaceAggregator.ProcessWorkspaceSymbol(ctx, clients, params)
            if err == nil && m.scipCache != nil {
                if m.isCacheableMethod(method) { _ = m.scipCache.Store(method, params, result) }
            }
            return result, err
        }
    ...
    if language == "" { return nil, fmt.Errorf("unsupported file type: %s", uri) }
    client, err := m.getClient(language)
    if err != nil { return nil, fmt.Errorf("no LSP client for language %s: %w", language, err) }
    ...
    if method != types.MethodWorkspaceSymbol { m.ensureDocumentOpen(client, uri, params) }
    result, err := m.sendRequestWithRetry(ctx, client, method, params, uri, language)
    if err == nil {
        if method == types.MethodTextDocumentReferences {
            if len(result) == 0 || string(result) == "null" { result = json.RawMessage("[]") }
        }
    }
    if err == nil && m.scipCache != nil && m.isCacheableMethod(method) {
        ...
        if method == types.MethodTextDocumentDocumentSymbol {
            idxCtx, cancel := common.CreateContext(constants.AdjustDurationForWindows(12*time.Second, 1.5)); defer cancel()
...
func (m *LSPManager) sendRequestWithRetry(ctx context.Context, client types.LSPClient, method string, params interface{}, uri string, language string) (json.RawMessage, error) {
    ...
    if runtime.GOOS == osWindows { baseDelay = 500 * time.Millisecond; maxRetries = 4 }
    ...
    for attempt := 0; attempt < maxRetries; attempt++ {
        ...
        if isContentModifiedRPCError(res) {
            if uri == "" || attempt == maxRetries-1 { return res, nil }
            delay := time.Duration(attempt+1) * baseDelay
            time.Sleep(delay)
            m.ensureDocumentOpen(client, uri, params)
            continue
        }
        if !isNoViewsRPCError(res) { return res, nil }
        if uri == "" || attempt == maxRetries-1 { return res, nil }
        if attempt == 0 { common.LSPLogger.Debug("Encountered 'no views' error for %s, retrying...", uri) }
        m.ensureDocumentOpen(client, uri, params)
        delay := time.Duration(attempt+1) * baseDelay
        var b [1]byte; _, _ = cryptoRand.Read(b[:])
...
func isNoViewsRPCError(raw json.RawMessage) bool {
    ...
    var e struct{ Code int `json:"code"`; Message string `json:"message"` }
    if err := json.Unmarshal(raw, &e); err != nil { return false }
