...
type ClientStatus struct { Active bool; Error error; Available bool }

type serverConfigWrapper struct { language string; config *config.ServerConfig }
func (w *serverConfigWrapper) Start(ctx context.Context) error { return nil }
func (w *serverConfigWrapper) Stop() error { return nil }
func (w *serverConfigWrapper) SendRequest(ctx context.Context, method string, params interface{}) (json.RawMessage, error) { return nil, nil }
func (w *serverConfigWrapper) SendNotification(ctx context.Context, method string, params interface{}) error { return nil }
...
func (m *LSPManager) CheckServerAvailability() map[string]ClientStatus {
    ...
    for lang, client := range m.clients {
        if _, ok := result[lang]; !ok { continue }
...
func (m *LSPManager) startClientWithTimeout(ctx context.Context, language string, cfg *config.ServerConfig) error {
        ...
        if runtime.GOOS == osWindows || isFWCD {
            ...
            if err == nil {
                if errStart := socketClient.Start(ctx); errStart == nil {
                    m.mu.Lock(); m.clients[language] = socketClient; m.mu.Unlock()
                    if activeClient, ok := socketClient.(interface{ IsActive() bool }); ok {
                        ...
                        for i := 0; i < maxWaitIterations; i++ {
                            if activeClient.IsActive() { break }
            ...
            if err := security.ValidateCommand(resolvedCommand, stdioArgs); err == nil {
                if stdClient, e2 := NewStdioClient(stdioCfg, language); e2 == nil {
                    if e3 := stdClient.Start(ctx); e3 == nil {
                        m.mu.Lock(); m.clients[language] = stdClient; m.mu.Unlock()
                        return nil
                    } else {
                ...
                if err := security.ValidateCommand(p2, nil); err == nil {
                    if fwcdClient, e3 := NewStdioClient(altCfg, language); e3 == nil {
                        if e4 := fwcdClient.Start(ctx); e4 == nil {
                            ...
                            m.mu.Lock(); m.clients[language] = fwcdClient; m.mu.Unlock()
                            return nil
                        } else {
    ...
    if _, err := exec.LookPath(resolvedCommand); err != nil {
        if language == langPython {
            type candidate struct{ cmd string; args []string }
            seen := map[string]bool{}
            candidates := []candidate{}
            add := func(cmd string, args []string) { if cmd != "" && !seen[cmd] { seen[cmd] = true; candidates = append(candidates, candidate{cmd: cmd, args: args}) } }
    ...
    if err := client.Start(ctx); err != nil { return fmt.Errorf("failed to start client: %w", err) }

    m.mu.Lock(); m.clients[language] = client; m.mu.Unlock()
    ...
    if activeClient, ok := client.(interface{ IsActive() bool }); ok {
        ...
        for i := 0; i < maxWaitIterations; i++ {
            ...
            default:
                if activeClient.IsActive() { return nil }
                time.Sleep(100 * time.Millisecond)
            }
        }
...
func (m *LSPManager) getClient(language string) (types.LSPClient, error) {
    ...
    if !exists { return nil, fmt.Errorf("no client for language: %s", language) }
    return client, nil
}

func (m *LSPManager) GetClient(language string) (types.LSPClient, error) { return m.getClient(language) }
...
func (m *LSPManager) detectPrimaryLanguage(workingDir string) string {
    projectMarkers := []struct{ files []string; language string }{
    ...
    if files, err := os.ReadDir(workingDir); err == nil {
        ...
        for _, file := range files {
            ...
            switch ext {
            case ".go": langCounts["go"]++
            case ".py": langCounts["python"]++
            case ".js", ".mjs": langCounts["javascript"]++
            case ".ts": langCounts["typescript"]++
            case ".java": langCounts["java"]++
            case ".rs": langCounts["rust"]++
            }
        }
        maxCount := 0; var primaryLang string
        for lang, count := range langCounts { if count > maxCount { maxCount = count; primaryLang = lang } }
